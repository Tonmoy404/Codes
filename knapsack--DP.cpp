...
vector<ii>jhinuk;
int totalJhinuk, totalWeight;
int dp[100][1000];

int call(int i, int w){
    if(i>=totalJhinuk) return 0;

    if(dp[i][w]!= -1) return dp[i][w];

    if(jhinuk[i].first<=w) return dp[i][w] = max(jhinuk[i].second+call(i+1, w-jhinuk[i].first), call(i+1, w));

    return dp[i][w] = call(i+1, w);

}

int main(){
    mem(dp, -1);
    cout<<"Enter number of total Jhinuk & total Weight: ";
    cin>>totalJhinuk>>totalWeight;
    for(int i=0; i<totalJhinuk; i++){
        int w, p; cin>>w>>p;
        jhinuk.pb(ii(w, p));
    }

    cout<<"Maximum possible price is: "<<call(0, totalWeight)<<endl;
    return Accepted;
}
