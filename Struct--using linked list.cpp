...
struct node{   //making new data type consists of value and address part
    int val;
    node *link;
};

node *top = NULL;  //initially making top as NULL

void push(int data){
    node *temp = new node();   //making a node named temp
    temp->val = data;   //assigning value to the data part
    temp->link = top;  //linking the top to the new node
    top = temp;  //making new node as top
}

void pop()
{
    if(top==NULL){   //checking if top is NULL or not
        return ;
    }
    node *temp = top;   //making a reference variable (temp) of top
    top = top->link;   //making the next node as top
    delete temp;   //making free the memory
}

void print(){   //printing values of stack
    while(top!=NULL){
        cout<<top->val<<" ";
        pop();
    }
